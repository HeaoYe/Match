...
namespace Match {
    ...
    public:
        MATCH_API VertexAttributeSet(const std::vector<InputBindingInfo> &binding_infos);
        MATCH_API ~VertexAttributeSet();
    private:
        MATCH_API void add_input_binding(const InputBindingInfo &binding_info);
    INNER_VISIBLE:
        uint32_t location = 0;
