...
namespace Match {
    StorageImage::StorageImage(uint32_t width, uint32_t height, bool sampled) {
        vk::ImageUsageFlags usage = vk::ImageUsageFlagBits::eStorage;
        if (sampled) {
            usage |= vk::ImageUsageFlagBits::eSampled;
        }
        image = std::make_unique<Image>(width, height, vk::Format::eR8G8B8A8Snorm, usage, vk::SampleCountFlagBits::e1, VMA_MEMORY_USAGE_GPU_ONLY, VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT, 1);
        ...
        image_view = create_image_view(image->image, vk::Format::eR8G8B8A8Snorm, vk::ImageAspectFlagBits::eColor, 1);
    }
